	...
	<div class="themesflat-container">
		<div class="row">
			<div class="col-xl-8 col-lg-9 col-md-8 col-12">
				<div class="box-activity">
								...
								<div class="content">
									...
									<div class="infor">
										...
										<div class="delete-button"
											onclick="deleteCart(${cart.cartId.user.id },${cart.cartId.product.id })"
					...
					<c:if test="${not empty product }">
						<!-- item -->
						<div id="card-${product.id }" class="sc-card-activity style-2"
							style="<c:if test='${product.isSelected }'>background-color: #393966;</c:if>">
							<div class="content">
								<div class="checkbox" style="align-content: center;">
								...
								<div class="media">
									<img
										src="<c:url value='${product.productImages.get(o).product_img_name }'/>"
										alt="">
								...
								<div class="infor">
									...
									<div class="status">
										Brand: <span class="author">${product.brand.name }</span>
									</div>
									<div class="status">
										Price:
										<c:choose>
											<c:when test="${not empty product.priceSale }">
												<span class="author" id="price-${product.id }"><del>
														<fmt:formatNumber type="number" maxFractionDigits="0"
															value="${product.price}" />
														VND
													</del> -${Math.round((product.price/product.priceSale - 1)*100)}
													%</span>
												<span id="sale-${product.id }"> <fmt:formatNumber
														type="number" maxFractionDigits="0"
														value="${product.priceSale*product.value }" /> VND
												</span>
											</c:when>
											<c:otherwise>
												<span class="author" id="price-${cart.cartId.product.id }"></span>
												<span id="sale-${product.id }">${product.price*cart.value }
													VND</span>
											</c:otherwise>
										</c:choose>
									</div>
									...
									<div class="time">
										Qty: <input type="number" id="val-${product.id }"
											onchange="updateCart(${user.id },${product.id })"
											style="height: 3rem; width: 15rem;" value="${product.value}">
									</div>

									<div class="delete-button"
										onclick="deleteCart(${user.id },${product.id })"
										style="background-color: red; padding: 1rem; border-radius: 1rem; align-content: center; margin-top: 1rem; font-size: 2rem; min-width: 3rem">
										<i style="font-size: 2rem;" class="fas fa-trash"> </i> Delete
									</div>
								</div>
			...
			<div class="col-xl-4 col-lg-3 col-md-4 col-12">
				...
				<form id="order-comfirm" action="<c:url value='/save-orders'/>" method="post">					
						<div class="form-group">
							<label class="mb-1 text-white " style="font-size: 2rem"><strong>Delivery Name</strong></label> 
							<input style="font-size: 1.5rem; padding: 0.5rem;"
								type="text" name="delivery_name" class="form-control" value=""
								placeholder="Your Account">
						</div>
						<div class="form-group">
							<label class="mb-1 text-white" style="font-size: 2rem"><strong>Delivery Phone</strong></label> 
							<input style="font-size: 1.5rem; padding: 0.5rem;"
								type="text" name="delivery_phone" class="form-control"
								value="" placeholder="Password">
						</div>
						<div class="form-group">
							<label class="mb-1 text-white" style="font-size: 2rem"><strong>Delivery Address</strong></label> 
							<input style="font-size: 1.5rem; padding: 0.5rem;"
								type="text" name="delivery_address" class="form-control"
								value="" placeholder="Password">
						</div>
						<div class="form-group">
							<label class="mb-1 text-white" style="font-size: 2rem"><strong>Delivery Email</strong></label> 
							<input style="font-size: 1.5rem; padding: 0.5rem;"
								type="text" name="delivery_email" class="form-control"
								value="" placeholder="Password">
						</div>
						<div class="form-group">
							<label class="mb-1 text-white" style="font-size: 2rem"><strong>Payment Method</strong></label> 
							<input style="font-size: 1.5rem; padding: 0.5rem;"
								type="text" name="delivery_method" class="form-control"
								value="" placeholder="Password">
						</div>
						<div class="form-group">
							<label class="mb-1 text-white" style="font-size: 2rem"><strong>Notes</strong></label> 
							<input style="font-size: 1.5rem; padding: 0.5rem;"
								type="text" name="delivery_note" class="form-control"
								value="" placeholder="Password">
						</div>
						
						<div class="col-md-12 wrap-inner load-more text-center mg-t16" >
							<button class="sc-button fl-button pri-3"><span>Confirm
									Order</span></button>
						</div>
					</form>
